...
wait_time = 3
page_size = 100
sort_list = ['hotness', 'commentCount', 'dateCreated', 'scoreDescending', 'viewCount', 'voteCount']

for sort in sort_list:
    from_list = []
    page = 1
    kernel_list = kaggle.api.kernels_list_with_http_info(page=page, page_size=page_size, language="python", sort_by=sort)
        ...
        with tqdm(total=kernel_len, desc=f"Page {page} for {sort}") as pbar:
            for i in range(page_size):
                ...
                for attempt in range(max_retries):
                    ...
                    raise ConnectionError(f"Failed to connect to {kernel_url} after {max_retries} attempts.")
                kernel_soup = BeautifulSoup(kernel_data.text, 'html.parser')

                render_soup = kernel_soup.find_all('script', {'class': 'kaggle-component'})
                ...
                for attempt in range(max_retries):
                    ...
                    raise ConnectionError(f"Failed to connect to {render_url} after {max_retries} attempts.")
                notebook_soup = BeautifulSoup(notebook_data.text, 'html.parser')
                notebook_set = set()
                for div in notebook_soup.find_all('div', {'class': ['highlight hl-ipython3', 'highlight hl-lexer_wrapper']}):
                    import_text = [x.text for x in  div.find_all('span', {'class': 'kn'})]
                    if 'import' in import_text:
                        library_text = [x.text for x in div.find_all('span', {'class': ['k', 'kn', 'n', 'nn']})]
                        from_switch = False
